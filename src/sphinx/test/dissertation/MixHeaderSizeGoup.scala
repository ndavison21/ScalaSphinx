...
object MixHeaderSizeGroup {
  
  def main(args: Array[String]) {
    Framework.writeToFile("output/mixHeaderSizeGroup.csv", "r,header-size\n")
    for (r <- 1 to 100) {
      Framework.cleanup
      ...
      Framework.appendToFile("output/mixHeaderSizeGroup.csv", r + "," + overhead)
    }
  }  
}