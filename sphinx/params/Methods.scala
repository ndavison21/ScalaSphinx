...
object Methods {
  ...
  def dEnc(destination: Array[Byte]): String = {
    ...
    def byteArrayToString(i: Int, byteArray: Array[Byte], string: String): String = {
      if (i == byteArray.length) string
      else
        byteArrayToString(i+1, byteArray, string + byteArray(i))
    }
    
    return destination.length.toChar + byteArrayToString(0, destination, "")
  }
